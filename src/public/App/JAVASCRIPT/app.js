...
const fetchPrice = fetch('/App/API_PRICE')
const fetchHistorial = fetch('/App/Get-Historial')
const fetchInflation = fetch('/App/API_INFLATION')
...
Promise.all([fetchData, fetchPrice, fetchHistorial, fetchInflation])
  .then(responses => {
    ...
    const dataPromise4 = response4.json()

    return Promise.all([dataPromise1, dataPromise2, dataPromise3, dataPromise4])
  })
  .then(results => {
    ...
    const DollarPrce = res2.toFixed(2)
...
function DollarPrice(res){
    const precio = res.toFixed(2)
    
    const Price = document.createElement('h2')
    Price.innerHTML = `<h2>${precio} Bs</h2>`
